...
export default async function handler(res:NextApiResponse,req:NextApiRequest) {
    if (req.method !== "GET") {
        return res.status(405).json({ body : {message : "Method not allowed"} });
    }
    try {
        ...
        const overall = await sheets.spreadsheets.values.get({
            spreadsheetId: process.env.Google_Client_Id_score,
            range: "Overall!B2:B8",
        });
        const Culti = await sheets.spreadsheets.values.get({
            spreadsheetId: process.env.Google_Client_Id_score,
            range: "Culti!B2:B8",
        });
        const Sports = await sheets.spreadsheets.values.get({
            spreadsheetId: process.env.Google_Client_Id_score,
            range: "sports!B2:B8",
        });
        ...
        return res.status(200).json({ body : {score} });
    } catch (error) {
